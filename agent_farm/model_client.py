...
def _default_transport(
    ...
    try:
        ...
        raise ModelClientError(f"Model endpoint returned HTTP {exc.code}: {raw}") from exc
    except URLError as exc:
        raise ModelClientError(f"Could not reach the model endpoint: {exc.reason}") from exc
    except TimeoutError as exc:
        raise ModelClientError("The model endpoint timed out.") from exc
    try:
