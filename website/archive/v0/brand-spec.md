...
- Product capture: intentionally marked as pending in v0. The full build must replace the concept
  surfaces with a current local capture from the real Agent Farm product, with sensitive data removed.
...
- Concept UI must be labeled as concept UI until it is replaced by a real local product capture.
