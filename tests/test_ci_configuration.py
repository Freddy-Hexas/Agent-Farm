...
class ContinuousIntegrationConfigurationTests(unittest.TestCase):
    def test_ci_runs_python_native_build_and_real_native_ui_automation(self) -> None:
        ...
        self.assertIn("AGENT_FARM_SOURCE_ROOT", workflow)
        self.assertIn("AGENT_FARM_PYTHON", workflow)
        self.assertIn("wait-for RepositoryButton", workflow)
        self.assertIn("Native UI startup", workflow)
